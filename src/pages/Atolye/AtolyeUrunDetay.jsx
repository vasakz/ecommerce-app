...
function AtolyeUrunDetay() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 md:py-12 bg-white relative">
                ...
                {tumGorseller.map((img, idx) => (
                  <button
                    ...
                    className={`w-16 h-16 md:w-20 md:h-20 rounded-md overflow-hidden border-2 transition-all flex-shrink-0 ${
                      seciliGorsel === idx ? 'border-amber-500' : 'border-transparent opacity-60 hover:opacity-100'
                    }`}
                  >
        ...
        <div className="lg:col-span-5 flex flex-col">
          <div className="lg:sticky lg:top-24 h-fit pb-8">
            ...
            <div className="mb-6">
              ...
              {/* Başlık ve Kalp (Favori) Butonu Aynı Satırda */}
              <div className="flex justify-between items-start gap-4 mb-4">
                <h1 className="text-3xl md:text-4xl font-serif text-stone-800 leading-tight">{urun.isim}</h1>
                <button 
                  onClick={favoriToggle}
                  className={`p-3 rounded-full flex-shrink-0 transition-all ${isFavorited ? 'bg-rose-50 text-rose-500' : 'bg-stone-50 text-stone-400 hover:text-rose-500 hover:bg-rose-50'}`}
                  title={isFavorited ? "Favorilerden Çıkar" : "Favorilere Ekle"}
                >
                  <svg xmlns="http://www.w3.org/2000/svg" fill={isFavorited ? "currentColor" : "none"} viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
            ...
            {/* Boyut Seçimi - AMBER-500 Renk Uygulandı */}
            <div className="mb-8">
              ...
              <div className="grid grid-cols-3 gap-3">
                {BOYUTLAR.map((b) => (
                  ...
                  >
                    ...
                    <span className={`text-[10px] ${seciliBoyut?.id === b.id ? 'text-amber-50' : 'text-stone-400'}`}>
              ...
              {seciliBoyut?.id === 'xl' && (
                <div className="mt-3 p-4 bg-stone-50 border border-stone-200 rounded-md transition-all">
                  <input 
                    type="text" 
                    value={ozelOlcu}
              ...
              <div className="flex gap-4">
                {RENKLER.map((r) => (
                  <button key={r.id} onClick={() => setSeciliRenk(r)} className="flex flex-col items-center gap-2 group">
                    <div className={`w-8 h-8 rounded-full p-0.5 border-2 transition-colors ${seciliRenk?.id === r.id ? 'border-amber-500' : 'border-transparent group-hover:border-stone-300'}`}>
                    ...
                    <div key={e.id} className="flex flex-col gap-2">
                      <button
                        ...
                        className={`flex items-center justify-between p-4 rounded-md border transition-all text-left ${
                          isSelected ? 'border-amber-500 bg-amber-50/50' : 'border-stone-200 hover:border-stone-300'
                        }`}
                      >
      ...
      {/* -- BAKIM KILAVUZU MODALI -- */}
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm z-[100]">
          <div className="bg-white p-8 rounded-md max-w-md w-full relative shadow-2xl">
            ...
            <p className="text-sm text-stone-600 leading-relaxed">
              Ürünün doğal yapısını koruması için kimyasal temizleyicilerden uzak tutun. Tozunu almak veya temizlemek için sadece hafif nemli, yumuşak pamuklu bir bez ile nazikçe silmeniz yeterlidir.
            </p>
            <button 
              onClick={() => setIsModalOpen(false)}
