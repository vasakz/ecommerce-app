...
function MiniUrunKarti({ urun, atolyeKategori }) {
  ...
  //  type:'atolye', gercekId ve orijinalUrun eklendi → Cart navigation artık çalışır
  const sepeteEkle = (e) => {
    e.stopPropagation()
    e.preventDefault()
...
function AtolyeVitrin({ atolye }) {
  ...
  const manuelKaydir = (yon) => {
    if (sliderRef.current) sliderRef.current.scrollBy({ left: yon * 280, behavior: 'smooth' })
  }
  ...
  return (
    <div className="mb-10 bg-white rounded-xl shadow-sm border border-stone-100 overflow-hidden">
      ...
      <div className="flex flex-col md:flex-row gap-6 items-start md:items-center px-8 pt-6 pb-4 border-b border-stone-100">
        ...
        <p className="text-stone-600 text-sm leading-relaxed flex-1 pt-2 md:pt-0">{atolye.aciklama}</p>
      ...
      <div className="px-8 py-6 relative group">
        ...
        >
          {atolye.urunler.map((urun, index) => (
            <MiniUrunKarti key={`${urun.id}-${index}`} urun={urun} atolyeKategori={atolye.kategori} />
          ))}
